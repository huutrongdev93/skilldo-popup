<div class="salebanner-component-3 widgetBackgroundImage">
    <div class="banner-infoSale widgetBgInfo">
        <div class="banner-infoText">
            <p class="">{{$config['title3']}}</p>
        </div>
    </div>
    <div class="banner-text">
        <div class="text-title">
            <div class="text-title1 widgetTitle">{{$config['title1']}}</div>
            <div class="text-title2 widgetBigTitle">{{$config['title2']}}</div>
        </div>
        <div class="text-des widgetDescription">{{$config['content']}}</div>
        <div class="button-link widgetLink">
            <a href="{{$config['btn_url']}}" class="btn button-href widgetButton" target="_top">{{$config['btn_txt']}}</a>
...
<style>
    ...
    .salebanner-component-3 .banner-infoSale {
        ...
        height: 120px;
        background-size: cover;
        background-image: url('{{ asset('popup::images/style3/heart-34.png') }}');
    }
    .salebanner-component-3 .banner-infoText {
        width: 92%;
        height: 100%;
        padding: 1px;
        color: {{$config['title3_color']}}!important;
