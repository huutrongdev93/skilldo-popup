    ...
    SlickModals.prototype = {
        ...
        _checkInitRestrictions: function () {
            ...
            if (self.settings.restrict_dateRange) {
                ...
                triggerOpen(!!(cookieExist() || hiddenOnPage() || hideOnMobile() || self._activeInstanceExist() || restrictedVisits() || hideAfterVisits()));
            }
        ...
        _togglePopup: function (action, timer, currentPopupDelay, currentOverlayDelay, currentContentDelay) {
            ...
            if (enable) {
                ...
                //console.log('time to close',(parseFloat(self.$popup.css('animation-delay')) + parseFloat(self.$popup.css('animation-duration'))) * 1000 + timer)
                setTimeout (function () {
        ...
        popupPosition: function (position) {
            ...
            var isMobile = $(window).width() <= parseInt(this.settings.mobile_breakpoint);
            //console.log('popup position check..',isMobile,$(window).width(),parseInt(this.settings.mobile_breakpoint));
            if(isMobile)
                return ;
        ...
        mobilePosition: function (position) {
            var isMobile = $(window).width() <= parseInt(this.settings.mobile_breakpoint);
            console.log('mobile position check..',isMobile);
            if(!isMobile)
                return ;    
            if (typeof position !== 'string') return _log(_LOG_START + 'Only string' + _LOG_END_1);
